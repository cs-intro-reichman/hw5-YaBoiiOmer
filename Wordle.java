public class Wordle {
    ...
    public static String chooseSecretWord(String[] dict) {
        int randIndex = (int) (Math.random() * dict.length);
    ...
    public static boolean containsChar(String secret, char c) {
        for(int i = 0; i< secret.length(); i++){
            if(secret.toUpperCase().charAt(i) == c) return true;
        }
        return false;
    }

    // Made this function to check if input is made out of characters only
    public static boolean validInput(String input){
        for(int i = 0; i < input.length(); i++){
            if(!(input.charAt(i) >= 'A' && input.charAt(i) <= 'Z')) return false;
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        while (attempt < MAX_ATTEMPTS && !won) {
            ...
            while (!valid) {
                ...
                if (guess.length() != WORD_LENGTH || !validInput(guess)) {
            ...
            computeFeedback(secret, guess, resultRow[attempt]);

            // Print board
            printBoard(guesses, resultRow, attempt);

            // Check win
            if (isAllGreen(resultRow[attempt])) {
